...
def load_data(ticker, n_steps=50, scale=True, shuffle=True, lookup_step=1, split_by_date=True,
    ...
    """
    Loads data from Bittrex source, as well as scaling, shuffling, normalizing and splitting.
    Params:
    ...
    """
    btx = Bittrex(os.environ.get("BITTREX_API_KEY"), os.environ.get("BITTREX_API_SECRET"))
    # see if ticker is already a loaded stock from yahoo finance
    if isinstance(ticker, str):
        # load it from yahoo_fin library
        raw_data = btx.markets_candle(ticker, interval=timedelta(days=1))
        max_interval = timedelta(days=366)
        df = _data2frame(raw_data['data'])
